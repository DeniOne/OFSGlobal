...
def test_get_employee(test_database, sample_employee_data):
    ...
    # Получаем данные
    employee = test_database.get_employee(employee_id)
    
    # Проверяем данные
    assert employee == sample_employee_data
...
def test_get_all_employees(test_database, sample_employee_data):
    ...
    employees = test_database.get_all_employees()
    
    # Проверяем количество и данные
    assert len(employees) == 3
    assert all(emp["name"].startswith("Сотрудник") for emp in employees)

def test_get_employees_by_competency(test_database, sample_employee_data):
    ...
    python_devs = test_database.get_employees_by_competency("Python")
    assert len(python_devs) == 1
    assert python_devs[0]["name"] == "Python разработчик"
    
    # Ищем Java разработчиков
    java_devs = test_database.get_employees_by_competency("Java")
