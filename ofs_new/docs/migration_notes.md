...
## Глобальная цель
Полное удаление моделей Employee и Department, переход на Staff и Division
...
- URL в браузере остаются `/employees` и возможно `/departments`
- В CRUD есть алиасы: `employee = staff` и `department = division`
- Таблица организации называется "organization" (в единственном числе)
- В модели `Employee` ссылка ForeignKey должна быть на "organization.id", а не "organizations.id"

## Важные места в коде
- `backend/app/crud/__init__.py` - содержит алиасы для обратной совместимости
- `backend/app/models/organization.py` - содержит отношения к staff и employees
- `backend/app/models/employee.py` - старая модель, содержит проблемные отношения
...
- Проверить связи в модели `FunctionalRelation`, она может использовать Employee вместо Staff
