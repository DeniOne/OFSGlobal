...
### Примечания для разработчиков
- Во всех новых разработках следует использовать термины `divisions` и `staff` вместо `departments` и `employees`
...
- Страница отделов доступна по маршруту `/divisions` с редиректом с `/departments`
