...
class Employee(Base):
    ...
    parent_id = Column(Integer, ForeignKey("employees.id"), nullable=True)
    ...
    parent = relationship("Employee", remote_side=[id], back_populates="children", uselist=False)
    children = relationship("Employee", back_populates="parent", remote_side=[parent_id])
    locations = relationship("Location", secondary="employee_locations", back_populates="employees")
