...
@router.get("/tree", response_model=List[schemas.DepartmentTree])
async def get_division_tree(
    ...
    """
    Получить древовидную структуру отделов организации.
    """
    tree = await crud.division.get_Division_tree(
        db, 
        organization_id=organization_id,
        include_inactive=include_inactive
    )
...
@router.get("/{division_id}/children", response_model=List[schemas.Department])
...
@router.put("/{division_id}/move", response_model=schemas.Department)
...
async def delete_division(
    *,
    ...
    if children:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Нельзя удалить отдел, имеющий дочерние отделы. Сначала удалите или переместите дочерние отделы."
        )
