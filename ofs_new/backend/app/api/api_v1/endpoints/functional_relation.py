...
from app.models.functional_relation import RelationType
...
def create_functional_relation(
    *,
    ...
    manager = crud.employee.get(db=db, id=manager_id)
    if not manager:
        raise HTTPException(status_code=404, detail="Руководитель не найден")
    
    # Проверяем существование подчиненного
    subordinate = crud.employee.get(db=db, id=relation_in.subordinate_id)
    if not subordinate:
...
def get_relations_by_subordinate(
    *,
    ...
    """
    # Проверяем существование подчиненного
    subordinate = crud.employee.get(db=db, id=subordinate_id)
    if not subordinate:
