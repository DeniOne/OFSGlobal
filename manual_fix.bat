@echo off
echo Удаление файлов с null-байтами...
cd D:\OFS_Global\ofs_project\ofs_new\backend\app\api\api_v1\endpoints

if exist departments.py del /F /Q departments.py
if exist employees.py del /F /Q employees.py

cd D:\OFS_Global\ofs_project\ofs_new\backend\app\api\api_v1
if exist api.py del /F /Q api.py

echo Файлы удалены успешно!
echo.
echo Создание новых файлов...
...
echo     departments, employees, functional_relations, telegram_bot, positions>> api.py
echo )>> api.py
echo.>> api.py
...
echo.
echo Запуск бэкенда...
cd D:\OFS_Global\ofs_project\ofs_new\backend
